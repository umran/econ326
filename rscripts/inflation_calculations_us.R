...
gdpdef_data <- read.table("http://127.0.0.1/GDPDEF.csv", header = TRUE, sep = ",")
...
ggplot(inflation_rates_cpi, aes(PERIOD, INFLATION, group = 1)) + 
  ...
  labs(title="Inflation as Measured by the CPI for the US")
...
ggplot(inflation_rates_gdpdef, aes(PERIOD, INFLATION, group = 1)) + 
  ...
  xlab("Time Period") + 
  ylab("Rate of Inflation (%)") +
  labs(title="Inflation as Measured by the GDP Deflator for the US")
